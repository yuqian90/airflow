...
# In 1.10.*, importing www_rbac.views requires create_app to be called first.
# The following is a workaround to make the import work in this test. This workaround
# can be removed in Airflow 2.0 and airflow.www_rbac.views can be imported directly.

@pytest.fixture(autouse=True, scope="module")
def _init_app():
    from airflow.www_rbac import app
    from airflow.settings import Session

    yield app.create_app(session=Session, testing=True)


def dag_edges(*args, **kwargs):
    from airflow.www_rbac.views import dag_edges

    return dag_edges(*args, **kwargs)


def task_group_to_dict(*args, **kwargs):
    from airflow.www_rbac.views import task_group_to_dict

    return task_group_to_dict(*args, **kwargs)
...
def test_sub_dag_task_group():
    ...
    subdag = dag.sub_dag(task_regex="task5", include_upstream=True, include_downstream=False, update_task_group=True)
    ...
    assert set(subdag_task_groups.keys()) == {None, "group234", "group234.group34"}
...
def test_duplicate_group_id():
    ...
    with pytest.warns(PendingDeprecationWarning, match=r".* group1 .*"):
        with DAG("test_duplicate_group_id", start_date=execution_date):
            with TaskGroup("group1", prefix_group_id=False):
                _ = DummyOperator(task_id="group1")

    with pytest.warns(PendingDeprecationWarning, match=r".* group1.downstream_join_id .*"):
        with DAG("test_duplicate_group_id", start_date=execution_date):
            _ = DummyOperator(task_id="task1")
            with TaskGroup("group1"):
                _ = DummyOperator(task_id="downstream_join_id")

    with pytest.warns(PendingDeprecationWarning, match=r".* group1.upstream_join_id .*"):
        with DAG("test_duplicate_group_id", start_date=execution_date):
            _ = DummyOperator(task_id="task1")
            with TaskGroup("group1"):
